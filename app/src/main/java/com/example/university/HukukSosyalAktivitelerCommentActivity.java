...
public class HukukSosyalAktivitelerCommentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hukuk_sosyal_aktiviteler_comment);
        Post_Key = getIntent().getExtras().get("PostKey2").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("HukukSosyalAktiviteler").child(Post_Key).child("HukukSosyalAktivitelerComments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(HukukSosyalAktivitelerCommentActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(HukukSosyalAktivitelerCommentActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(HukukSosyalAktivitelerCommentActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<HukukSosyalAktivitelerComment, HukukSosyalAktivitelerCommentViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<HukukSosyalAktivitelerComment, HukukSosyalAktivitelerCommentViewHolder>
                (HukukSosyalAktivitelerComment.class,
                        R.layout.all_hukuk_sosyal_aktiviteler_comment_layout,
                        HukukSosyalAktivitelerCommentViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(HukukSosyalAktivitelerCommentViewHolder viewHolder, HukukSosyalAktivitelerComment model, int position) {
    ...
    public static class HukukSosyalAktivitelerCommentViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public HukukSosyalAktivitelerCommentViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak
