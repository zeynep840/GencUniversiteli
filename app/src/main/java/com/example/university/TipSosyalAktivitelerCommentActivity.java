...
public class TipSosyalAktivitelerCommentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tip_sosyal_aktiviteler_comment);
        Post_Key = getIntent().getExtras().get("PostKey2").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("TipSosyalAktiviteler").child(Post_Key).child("TipSosyalAktivitelerComments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(TipSosyalAktivitelerCommentActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(TipSosyalAktivitelerCommentActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(TipSosyalAktivitelerCommentActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<TipSosyalAktivitelerComment, TipSosyalAktivitelerCommentViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<TipSosyalAktivitelerComment, TipSosyalAktivitelerCommentViewHolder>
                (TipSosyalAktivitelerComment.class,
                        R.layout.all_tip_sosyal_aktiviteler_comment_layout,
                        TipSosyalAktivitelerCommentViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(TipSosyalAktivitelerCommentViewHolder viewHolder, TipSosyalAktivitelerComment model, int position) {
    ...
    public static class TipSosyalAktivitelerCommentViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public TipSosyalAktivitelerCommentViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak
