...
public class EgitimSorCommentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_egitim_sor_comment);
        Post_Key = getIntent().getExtras().get("PostKey2").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("EgitimSor").child(Post_Key).child("EgitimSorComments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(EgitimSorCommentActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(EgitimSorCommentActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(EgitimSorCommentActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<EgitimSorComment, EgitimSorCommentViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<EgitimSorComment, EgitimSorCommentViewHolder>
                (EgitimSorComment.class,
                        R.layout.all_hukuk_sor_comment_layout,
                        EgitimSorCommentViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(EgitimSorCommentViewHolder viewHolder, EgitimSorComment model, int position) {
    ...
    public static class EgitimSorCommentViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public EgitimSorCommentViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak
