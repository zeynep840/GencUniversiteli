...
public class TipSorCommentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tip_sor_comment);
        Post_Key = getIntent().getExtras().get("PostKey2").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("TipSor").child(Post_Key).child("TipSorComments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(TipSorCommentActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(TipSorCommentActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(TipSorCommentActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<TipSorComment, TipSorCommentViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<TipSorComment, TipSorCommentViewHolder>
                (TipSorComment.class,
                        R.layout.all_hukuk_sor_comment_layout,
                        TipSorCommentViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(TipSorCommentViewHolder viewHolder, TipSorComment model, int position) {
    ...
    public static class TipSorCommentViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public TipSorCommentViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak
