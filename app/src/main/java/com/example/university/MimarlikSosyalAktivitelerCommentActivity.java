...
public class MimarlikSosyalAktivitelerCommentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mimarlik_sosyal_aktiviteler_comment);
        Post_Key = getIntent().getExtras().get("PostKey2").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("MimarlikSosyalAktiviteler").child(Post_Key).child("MimarlikSosyalAktivitelerComments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(MimarlikSosyalAktivitelerCommentActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(MimarlikSosyalAktivitelerCommentActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(MimarlikSosyalAktivitelerCommentActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<MimarlikSosyalAktivitelerComment, MimarlikSosyalAktivitelerCommentViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<MimarlikSosyalAktivitelerComment, MimarlikSosyalAktivitelerCommentViewHolder>
                (MimarlikSosyalAktivitelerComment.class,
                        R.layout.all_mimarlik_sosyal_aktiviteler_comment_layout,
                        MimarlikSosyalAktivitelerCommentViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(MimarlikSosyalAktivitelerCommentViewHolder viewHolder, MimarlikSosyalAktivitelerComment model, int position) {
    ...
    public static class MimarlikSosyalAktivitelerCommentViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public MimarlikSosyalAktivitelerCommentViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak
