...
public class VeterinerlikProjeOnerileriCommentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_veterinerlik_proje_onerileri_comment);
        Post_Key = getIntent().getExtras().get("PostKey2").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("VeterinerlikProjeOnerileri").child(Post_Key).child("VeterinerlikProjeOnerileriComments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(VeterinerlikProjeOnerileriCommentActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(VeterinerlikProjeOnerileriCommentActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(VeterinerlikProjeOnerileriCommentActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<VeterinerlikProjeOnerileriComment, VeterinerlikProjeOnerileriCommentViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<VeterinerlikProjeOnerileriComment, VeterinerlikProjeOnerileriCommentViewHolder>
                (VeterinerlikProjeOnerileriComment.class,
                        R.layout.all_veterinerlik_proje_onerileri_comment_layout,
                        VeterinerlikProjeOnerileriCommentViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(VeterinerlikProjeOnerileriCommentViewHolder viewHolder, VeterinerlikProjeOnerileriComment model, int position) {
    ...
    public static class VeterinerlikProjeOnerileriCommentViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public VeterinerlikProjeOnerileriCommentViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak
