...
public class IsletmeSosyalAktivitelerCommentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_isletme_sosyal_aktiviteler_comment);
        Post_Key = getIntent().getExtras().get("PostKey2").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("IsletmeSosyalAktiviteler").child(Post_Key).child("IsletmeSosyalAktivitelerComments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(IsletmeSosyalAktivitelerCommentActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(IsletmeSosyalAktivitelerCommentActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(IsletmeSosyalAktivitelerCommentActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<IsletmeSosyalAktivitelerComment, IsletmeSosyalAktivitelerCommentViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<IsletmeSosyalAktivitelerComment, IsletmeSosyalAktivitelerCommentViewHolder>
                (IsletmeSosyalAktivitelerComment.class,
                        R.layout.all_isletme_sosyal_aktiviteler_comment_layout,
                        IsletmeSosyalAktivitelerCommentViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(IsletmeSosyalAktivitelerCommentViewHolder viewHolder, IsletmeSosyalAktivitelerComment model, int position) {
    ...
    public static class IsletmeSosyalAktivitelerCommentViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public IsletmeSosyalAktivitelerCommentViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak
