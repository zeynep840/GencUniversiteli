...
public class EgitimSosyalAktivitelerCommentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_egitim_sosyal_aktiviteler_comment);
        Post_Key = getIntent().getExtras().get("PostKey2").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("EgitimSosyalAktiviteler").child(Post_Key).child("EgitimSosyalAktivitelerComments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(EgitimSosyalAktivitelerCommentActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(EgitimSosyalAktivitelerCommentActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(EgitimSosyalAktivitelerCommentActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<EgitimSosyalAktivitelerComment, EgitimSosyalAktivitelerCommentViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<EgitimSosyalAktivitelerComment, EgitimSosyalAktivitelerCommentViewHolder>
                (EgitimSosyalAktivitelerComment.class,
                        R.layout.all_egitim_sosyal_aktiviteler_comment_layout,
                        EgitimSosyalAktivitelerCommentViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(EgitimSosyalAktivitelerCommentViewHolder viewHolder, EgitimSosyalAktivitelerComment model, int position) {
    ...
    public static class EgitimSosyalAktivitelerCommentViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public EgitimSosyalAktivitelerCommentViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak
