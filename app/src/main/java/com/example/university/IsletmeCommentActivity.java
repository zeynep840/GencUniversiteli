...
public class IsletmeCommentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_isletme_comment);
        Post_Key = getIntent().getExtras().get("PostKey4").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("IsletmeSor").child(Post_Key).child("Comments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(IsletmeCommentActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(IsletmeCommentActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(IsletmeCommentActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<IsletmeComment,IsletmeCommentViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<IsletmeComment, IsletmeCommentViewHolder>
                (IsletmeComment.class,
                        R.layout.all_isletme_sor_comment_layout,
                        IsletmeCommentViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(IsletmeCommentViewHolder viewHolder, IsletmeComment model, int position) {
    ...
    public static class IsletmeCommentViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public IsletmeCommentViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak
