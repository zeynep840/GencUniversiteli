...
public class TipProjeOnerileriCommentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tip_proje_onerileri_comment);
        Post_Key = getIntent().getExtras().get("PostKey2").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("TipProjeOnerileri").child(Post_Key).child("TipProjeOnerileriComments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(TipProjeOnerileriCommentActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(TipProjeOnerileriCommentActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(TipProjeOnerileriCommentActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<TipProjeOnerileriComment, TipProjeOnerileriCommentViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<TipProjeOnerileriComment, TipProjeOnerileriCommentViewHolder>
                (TipProjeOnerileriComment.class,
                        R.layout.all_tip_proje_onerileri_comment_layout,
                        TipProjeOnerileriCommentViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(TipProjeOnerileriCommentViewHolder viewHolder, TipProjeOnerileriComment model, int position) {
    ...
    public static class TipProjeOnerileriCommentViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public TipProjeOnerileriCommentViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak
