...
public class HukukSorCommentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hukuk_sor_comment);
        Post_Key = getIntent().getExtras().get("PostKey2").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("HukukSor").child(Post_Key).child("HukukSorComments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(HukukSorCommentActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(HukukSorCommentActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(HukukSorCommentActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<HukukSorComment,HukukSorCommentViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<HukukSorComment, HukukSorCommentViewHolder>
                (HukukSorComment.class,
                        R.layout.all_hukuk_sor_comment_layout,
                        HukukSorCommentViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(HukukSorCommentViewHolder viewHolder,HukukSorComment model, int position) {
    ...
    public static class HukukSorCommentViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public HukukSorCommentViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak
