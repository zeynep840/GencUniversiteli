...
public class VeterinerlikSorCommentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_veterinerlik_sor_comment);
        Post_Key = getIntent().getExtras().get("PostKey2").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("VeterinerlikSor").child(Post_Key).child("VeterinerlikSorComments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(VeterinerlikSorCommentActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(VeterinerlikSorCommentActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(VeterinerlikSorCommentActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<VeterinerlikSorComment, VeterinerlikSorCommentViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<VeterinerlikSorComment, VeterinerlikSorCommentViewHolder>
                (VeterinerlikSorComment.class,
                        R.layout.all_veterinerlik_sor_comment_layout,
                        VeterinerlikSorCommentViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(VeterinerlikSorCommentViewHolder viewHolder, VeterinerlikSorComment model, int position) {
    ...
    public static class VeterinerlikSorCommentViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public VeterinerlikSorCommentViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak
