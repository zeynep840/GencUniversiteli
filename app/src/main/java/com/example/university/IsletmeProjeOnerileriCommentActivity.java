...
public class IsletmeProjeOnerileriCommentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_isletme_proje_onerileri_comment);
        Post_Key = getIntent().getExtras().get("PostKey2").toString();
        postsRef = FirebaseDatabase.getInstance().getReference().child("IsletmeProjeOnerileri").child(Post_Key).child("IsletmeProjeOnerileriComments");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            if (TextUtils.isEmpty(comments)) {
                                Toast.makeText(IsletmeProjeOnerileriCommentActivity.this, "Boş Yorum Gönderemezsin...", Toast.LENGTH_SHORT).show();
                            } else {
                                ...
                                postsRef.child(postRandom).updateChildren(commentsMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(IsletmeProjeOnerileriCommentActivity.this, "Başarılı bir şekilde yüklendi", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(IsletmeProjeOnerileriCommentActivity.this, "Eror", Toast.LENGTH_SHORT).show();
    ...
    protected void onStart() {
        ...
        super.onStart();
        FirebaseRecyclerAdapter<IsletmeProjeOnerileriComment, IsletmeProjeOnerileriCommentViewHolder> firebaseRecyclerAdapter=new FirebaseRecyclerAdapter<IsletmeProjeOnerileriComment, IsletmeProjeOnerileriCommentViewHolder>
                (IsletmeProjeOnerileriComment.class,
                        R.layout.all_isletme_proje_onerileri_comment_layout,
                        IsletmeProjeOnerileriCommentViewHolder.class,
                        postsRef

                )
        {
            @Override
            protected void populateViewHolder(IsletmeProjeOnerileriCommentViewHolder viewHolder, IsletmeProjeOnerileriComment model, int position) {
    ...
    public static class IsletmeProjeOnerileriCommentViewHolder extends RecyclerView.ViewHolder{
        View mView;

        public IsletmeProjeOnerileriCommentViewHolder(@NonNull View itemView) {
            super(itemView);
            mView=itemView;
        }
        //setler yazılacak
